...
export default function TicketPreview({ ticket, tournamentName, startingChips, onClose }: TicketPreviewProps) {
  ...
  const typeLabel = ticket.ticket_type === 'buyin' ? 'BUY-IN' : ticket.ticket_type === 'rebuy' ? 'RE-BUY' : 'ADD-ON';
      ...
      <div className="bg-white text-black rounded-lg p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
        <div ref={printRef}>
          <div className="text-center mb-3">
            ...
            <div className="text-xs text-gray-500">xPoker Tournament Director</div>
          </div>
          <div className="border-t border-dashed border-gray-400 my-3" />
          <div className="text-center font-bold text-2xl mb-2">{typeLabel}</div>
          <div className="space-y-1 text-sm">
            <div className="flex justify-between"><span>Ticket #:</span><span className="font-bold">{ticket.id}</span></div>
